...
## Overview
* finding and extracting raw data and getting it ready to analyze
...
1. **Raw Data**
    ...
    * did not manipulate, remove, or summarize in any way
2. **Tidy data set**
    ...
    * include variable names (that make sense)
...
3. **Code book describing each variable and its values in the tidy data set**
    ...
    * information about the summary choices that were made (e.g., median/mean)
...
* `grepl("text", object)` = searches through object and returns a T/F logical vector for if the text has been found
...
* `string` package [`library(stringr)`]
    ...
    * `substr("text", 1, 7)` = returns a substring of the specified beginning and ending positions
...
* **General Rules**
    ...
    * variables with character values
		* usually made into factor variables
		* descriptive
...
* `|` = or, used to combine subexpressions called alternatives (metacharacter)
    * *example*: `^([Gg]ood | [Bb]ad)` matches any lines that start with lower/upper "Good" or "Bad"
...
* `+` = 1 or more repetitions = 1 or more of some character/expression (metacharacter)
    * *example*: `[0-9]+` matches many at least 1 digit numbers such as "0", "90", or "021442132"
...
* `()` = define group as the matched text in parentheses, groups will be remembered and can be referred to by `\1`, `\2`, etc.
